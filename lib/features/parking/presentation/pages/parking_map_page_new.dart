...
class ParkingMapPageNew extends StatefulWidget {
  final ParkingModeConfig config;

  const ParkingMapPageNew({
    super.key,
    required this.config,
  });

  @override
  State<ParkingMapPageNew> createState() => _ParkingMapPageNewState();
}

class _ParkingMapPageNewState extends State<ParkingMapPageNew> {
  ...
  bool _followSearch = true; // Seguir la ubicación de búsqueda
  bool _isDriveMode = false;
  bool _isPrefetchingTiles = false;
  bool _isProgrammaticMove = false;
  bool _isRefreshing = false;
  bool _isGeocodingAddress = false;
  bool _isBottomSheetVisible = false; // Oculta por defecto
  ...
  Future<void> _initializeMode() async {
    if (widget.config.mode == ParkingMode.searchAddress) {
      await _geocodeAndCenterAddress();
    } else {
      await _initLocationTracking();
    }
  }

  Future<void> _geocodeAndCenterAddress() async {
    if (widget.config.searchAddress == null) return;

    setState(() {
      _isGeocodingAddress = true;
      _locationError = null; // Limpiar errores previos
    });

    List<Location>? locations;

    // Intentar con diferentes formatos de búsqueda
    try {
      locations = await locationFromAddress(
        '${widget.config.searchAddress}, Málaga, España',
      );
    } catch (_) {
      try {
        locations = await locationFromAddress(
          '${widget.config.searchAddress}, Málaga',
        );
      } catch (_) {
        try {
          locations = await locationFromAddress(
            widget.config.searchAddress!,
          );
        } catch (_) {
          ...
          setState(() {
            ...
            _locationError = 'No se pudo encontrar la dirección "${widget.config.searchAddress}". Intenta con otra dirección.';
          });
          return;
        }
      }
    ...
    // Si llegamos aquí, encontramos una ubicación
    if (locations.isNotEmpty && mounted) {
      final location = locations.first;
      final searchLatLng = LatLng(location.latitude, location.longitude);

      setState(() {
        _searchLocation = searchLatLng;
        _isGeocodingAddress = false;
        _locationError = null; // Asegurar que no hay error
        _followSearch = true; // Activar el seguimiento de la búsqueda
      });

      // Esperar un frame antes de mover la cámara para asegurar que el mapa está listo
      await Future.delayed(const Duration(milliseconds: 100));
      if (mounted) {
        _moveCamera(searchLatLng, zoom: 16);
      }
    } else {
      if (!mounted) return;
      setState(() {
        _isGeocodingAddress = false;
        _locationError = 'No se encontraron resultados para "${widget.config.searchAddress}".';
      });
    }
  }

  LatLng get _currentMapCenter {
    return _searchLocation ?? _userLocation ?? _initialCenter;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: _isDriveMode
          ...
          : AppBar(
              title: Text(
                widget.config.mode == ParkingMode.searchAddress
                    ? 'Parkings en ${widget.config.searchAddress}'
                    : 'Parkings cerca de mí',
              ),
              actions: [
                IconButton(
    ...
    return FlutterMap(
      ...
      options: MapOptions(
        ...
        onTap: (_, __) {
          ...
          // Ocultar la tarjeta al tocar el mapa
            ...
            markers: spots.map((spot) {
              ...
              return Marker(
                ...
                child: _ParkingMarker(
                  ...
                  size: size,
                  isAccessible: spot.hasAccessibleAccess,
                ),
              );
            }).toList(),
        ...
        if (_userLocation != null)
          MarkerLayer(
            markers: [
              Marker(
                point: _userLocation!,
                width: _isDriveMode ? 32 : 22,
                height: _isDriveMode ? 32 : 22,
                alignment: Alignment.center,
                child: _UserLocationMarker(
                  isFollowing: _followUser,
                  heading: _userHeading,
                  showHeading: _isDriveMode,
                ),
              ),
            ],
          ),
        if (_searchLocation != null && widget.config.mode == ParkingMode.searchAddress)
          MarkerLayer(
            markers: [
              Marker(
  ...
  Widget _buildBottomSheet() {
    if (_isDriveMode) return const SizedBox.shrink();

    final spots = _controller.spots;
    final centerPoint = _currentMapCenter;
    ...
    return AnimatedPositioned(
      ...
      left: 0,
      right: 0,
      bottom: _isBottomSheetVisible ? 0 : -sheetHeight,
      height: sheetHeight,
      child: Container(
        decoration: BoxDecoration(
        ...
        child: Column(
          ...
          children: [
            ...
            // Header con título y botón de ocultar
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Parkings cercanos (${spots.length})',
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _ParkingListItem(
                    spot: item.spot,
                    distance: item.distance,
                    onTap: () {
                      _onMarkerTap(item.spot);
                      // Ocultar la lista al seleccionar
                      setState(() {
                        _isBottomSheetVisible = false;
                      });
                    },
                    isSelected: item.spot.id == _controller.selectedSpot?.id,
                  );
                },
              ),
            ),
  ...
  Widget _buildFloatingButtons(BuildContext context) {
    ...
    return AnimatedPositioned(
      ...
      child: Column(
        ...
        children: [
          // Botón para mostrar/ocultar la lista de parkings
          if (!_isDriveMode) ...[
            FloatingActionButton.small(
              heroTag: 'toggleListFab',
              tooltip: _isBottomSheetVisible ? 'Ocultar lista' : 'Mostrar lista',
              onPressed: () {
                setState(() {
              ...
              child: Icon(
                _isBottomSheetVisible ? Icons.list_alt : Icons.list,
              ),
            ),
          ...
          const SizedBox(height: 12),
          if (widget.config.mode == ParkingMode.nearMe) ...[
            FloatingActionButton.small(
              heroTag: 'followFab',
              tooltip: _followUser
                  ? 'Dejar de seguirte'
          ...
          if (widget.config.mode == ParkingMode.searchAddress && _searchLocation != null) ...[
            FloatingActionButton.small(
              heroTag: 'centerSearchFab',
              tooltip: _followSearch
                  ? 'Dejar de seguir búsqueda'
                  : 'Centrar en búsqueda',
              backgroundColor: _followSearch
                  ? Theme.of(context).colorScheme.primary
                  : null,
              onPressed: _toggleFollowSearch,
              child: Icon(
                _followSearch ? Icons.place : Icons.place_outlined,
              ),
            ),
            const SizedBox(height: 12),
          ],
  ...
  Widget _buildSpotDetailsCard() {
    ...
    return AnimatedPositioned(
      ...
      right: 16,
      bottom: spot != null
          ? (_isBottomSheetVisible ? screenHeight * 0.35 : 24)
          : -screenHeight * 0.3, // Ocultar fuera de pantalla de forma responsive
      child: spot == null
  ...
  Future<void> _initLocationTracking() async {
    ...
    try {
      ...
      if (_followUser && _userLocation != null && widget.config.mode == ParkingMode.nearMe) {
        _moveCamera(current, zoom: _isDriveMode ? 17 : 15);
      }
    } catch (_) {
      if (!mounted) return;
      setState(() {
    ...
    _positionSubscription =
        Geolocator.getPositionStream(
          ...
          (position) {
            ...
            if (_followUser && widget.config.mode == ParkingMode.nearMe) {
              _moveCamera(
                current,
                zoom: _isDriveMode ? 17 : null,
                rotation: _isDriveMode ? position.heading : null,
              );
            }
          },
          onError: (_) {
  ...
  void _handleMapEvent(MapEvent event) {
    ...
    // Si no está siguiendo ni al usuario ni la búsqueda, no hacer nada
    if (!_followUser && !_followSearch) {
      return;
    }
    ...
    if (_isDriveMode) {
      ...
      if (_userLocation != null) {
        _moveCamera(
          _userLocation!,
          zoom: 17,
          rotation: _userHeading ?? 0,
        );
      }
    } else {
      SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge);
      // Reset rotation when exiting drive mode
      _moveCamera(
  ...
  void _toggleFollowSearch() {
    setState(() {
      _followSearch = !_followSearch;
    });

    if (_followSearch && _searchLocation != null) {
      _controller.selectSpot(null);
      _moveCamera(_searchLocation!, zoom: 16);
    }
  }
  ...
  void _moveCamera(LatLng target, {double? zoom, double? rotation}) {
    final double currentZoom = _mapController.camera.zoom;
    final double currentRotation = _mapController.camera.rotation;
  ...
  Future<void> _handleRefresh() async {
    ...
    await _controller.loadSpots(forceRefresh: true);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Plazas actualizadas'),
...
class _ParkingListItem extends StatelessWidget {
  final ParkingSpot spot;
  final double distance;
  final VoidCallback onTap;
  final bool isSelected;

  const _ParkingListItem({
    required this.spot,
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: LinearGradient(
                  ...
                  colors: spot.hasAccessibleAccess
                      ? const [Color(0xFFFFD700), Color(0xFFFFA500)]
                      : const [Color(0xFF0277BD), Color(0xFF29B6F6)],
                ),
              ),
              child: Icon(
                spot.hasAccessibleAccess ? Icons.accessible_forward : Icons.local_parking,
                color: Colors.white,
                size: 22,
              ),
            ),
    ...
    if (showHeading && heading != null) {
      return Transform.rotate(
        ...
        child: CustomPaint(
          ...
          painter: _ArrowPainter(color: accent),
        ),
      );
    }
...
class _ArrowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.moveTo(centerX, centerY - 12); // Top point
    path.lineTo(centerX + 8, centerY + 8); // Right point
    path.lineTo(centerX, centerY + 4); // Bottom center
    path.lineTo(centerX - 8, centerY + 8); // Left point
    path.close();
